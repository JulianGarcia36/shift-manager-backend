...
export default function EmployeePortal() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const resEmp = await fetch('http://127.0.0.1:8000/api/employees');
        const allEmps = await resEmp.json();
        const myEmp = allEmps.find(e => e.id === parseInt(employeeId));
        setEmployee(myEmp);

        const resShifts = await fetch('http://127.0.0.1:8000/api/shifts');
  ...
  const handleRequestSwap = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch('http://127.0.0.1:8000/api/public-shift-swaps', {
        method: 'POST',
        headers: { 
