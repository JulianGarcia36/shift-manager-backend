...
export default function SettingsView() {
  ...
  useEffect(() => {
    if (activeTab === 'security') {
      fetch('http://${import.meta.env.VITE_API_URL}/api/users', {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        const resSettings = await fetch('http://${import.meta.env.VITE_API_URL}/api/settings', { headers });
        if (resSettings.ok) {
          const dataSettings = await resSettings.json();
          if (dataSettings.id) setFormData(dataSettings);
        }

        const resTypes = await fetch('http://${import.meta.env.VITE_API_URL}/api/shift-types', { headers });
        if (resTypes.ok) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://${import.meta.env.VITE_API_URL}/api/settings', {
        method: 'PUT',
        headers: { 
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://${import.meta.env.VITE_API_URL}/api/user/password', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  const handleDeleteSubAdmin = async (id) => {
    ...
    const token = localStorage.getItem('token');
    await fetch(`http://${import.meta.env.VITE_API_URL}/api/users/${id}`, {
      method: 'DELETE',
  ...
  const handleAddShiftType = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://${import.meta.env.VITE_API_URL}/api/shift-types', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Accept': 'application/json', 
  ...
  const handleDeleteShiftType = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`http://${import.meta.env.VITE_API_URL}/api/shift-types/${id}`, {
        method: 'DELETE',
        headers: { 
          'Accept': 'application/json',
          'Authorization': `Bearer ${token}` 
