...
export default function SettingsView() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        const resSettings = await fetch('http://127.0.0.1:8000/api/settings', { headers });
        if (resSettings.ok) {
  ...
  const handleAddShiftType = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://127.0.0.1:8000/api/shift-types', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Accept': 'application/json', 
  ...
  const handleDeleteShiftType = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`http://127.0.0.1:8000/api/shift-types/${id}`, {
        method: 'DELETE',
        headers: { 
          'Accept': 'application/json',
          'Authorization': `Bearer ${token}` 
