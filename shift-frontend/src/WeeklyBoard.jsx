...
export default function WeeklyBoard() {
  ...
  const loadData = async () => {
    try {
      ...
      const resEmp = await fetch('http://127.0.0.1:8000/api/employees', { headers });
      setEmployees(await resEmp.json());
      
      const resShifts = await fetch('http://127.0.0.1:8000/api/shifts', { headers });
      setShifts(await resShifts.json());

      const resTypes = await fetch('http://127.0.0.1:8000/api/shift-types', { headers });
  ...
  const handleDeleteShift = async (shiftId) => {
    ...
    const token = localStorage.getItem('token');
    try {
      await fetch(`http://127.0.0.1:8000/api/shifts/${shiftId}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` }});
  ...
  const handleEventChange = async (info) => {
    ...
    const token = localStorage.getItem('token');
    await fetch(`http://127.0.0.1:8000/api/shifts/${shiftId}`, {
      method: 'PUT',
