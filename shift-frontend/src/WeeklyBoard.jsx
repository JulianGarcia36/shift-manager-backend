...
export default function WeeklyBoard() {
  ...
  const loadData = async () => {
    try {
      ...
      const resEmp = await fetch('http://${import.meta.env.VITE_API_URL}/api/employees', { headers });
      setEmployees(await resEmp.json());
      
      const resShifts = await fetch('http://${import.meta.env.VITE_API_URL}/api/shifts', { headers });
  ...
  const handleDeleteShift = async (shiftId) => {
    ...
    const token = localStorage.getItem('token');
    try {
      await fetch(`http://${import.meta.env.VITE_API_URL}/api/shifts/${shiftId}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` }});
  ...
  const handleEventChange = async (info) => {
    ...
    const token = localStorage.getItem('token');
    await fetch(`http://${import.meta.env.VITE_API_URL}/api/shifts/${shiftId}`, {
      method: 'PUT',
  ...
  const handleCreateShift = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      await fetch('http://${import.meta.env.VITE_API_URL}/api/shifts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
